...
Documentation
********************
``cell-ann`` is a framework for bridging data held in `AnnData` - the most popular single-cell python-based data structure and companion to scanpy - to the PyTorch ``Dataset`` class. ``torch-adata`` is meant to be structured but flexible within the rules of both of these data structures while also being easy to use.
...
.. code-block:: bash
    ...
    git clone https://github.com/mvinyard/cell-ann.git; cd cell-ann
    pip install -e .
